...
3. 从一个公司项目的 ts 代码改写而来。用 ts 做类似封装，需要动用“类”的概念，将它定义为一个单例，然后添加静态属性、方法，权限控制方面用 private、public 关键字控制。对于JS 来说，封装由闭包实现，具体来说，先开启一个函数作用域，如function(){ // 开启了一个函数作用域 }，函数内部实现功能逻辑，函数最后只返回供外界调用的方法，执行这个函数，并把结果赋给一个变量HTTPHelper，调用这个 HTTPHelper 对象的 GET、POST 方法就能分别发起不同请求。
4. 怎么把这个 HTTPHelper 导出来在其他地方使用？
*/